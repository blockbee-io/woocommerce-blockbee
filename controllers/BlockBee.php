...
class WC_BlockBee_Gateway extends WC_Payment_Gateway {
	...
	function __construct() {
		...
		$this->method_title       = 'BlockBee Payment Gateway';
		$this->method_description = __( 'BlockBee allows customers to pay in cryptocurrency', 'blockbee' );
	...
	function admin_options() {
        ...
        <div style='margin-top: .5rem;'>
			<?php echo __( "If you enjoy this plugin please <b><a href='https://wordpress.org/support/plugin/blockbee-payment-gateway-for-woocommerce/reviews/#new-post' target='_blank'>rate and review it</a></b>!", "blockbee" ) ?>
        </div>
		<?php
	}
		...
		if ( ! empty( WC_BlockBee_Gateway::$COIN_OPTIONS ) ) {
			...
			$this->form_fields = array(
				...
				'title'                      => array(
					...
					'default'     => __( 'Cryptocurrency', 'blockbee' ),
					'desc_tip'    => true,
				),
				'description'                => array(
	...
	public function get_icon() {
		$icon = '<img style="position:relative" width="120" src="' . BLOCKBEE_PLUGIN_URL . 'static/files/blockbee_logo.png' . '" alt="' . esc_attr( $this->get_title() ) . '" />';
        ...
        <div class="form-row form-row-wide">
            ...
            <ul style="margin-top: 7px; list-style: none outside;">
				...
				if ( ! empty( $this->coins ) && is_array( $this->coins ) ) {
					...
					?>
                    <li>
                        <select name="blockbee_coin" id="payment_blockbee_coin" class="input-control" style="display:block; margin-top: 10px">
                            <option value="none"><?php echo __( 'Please select a Cryptocurrency', 'blockbee' ) ?></option>
							<?php
							foreach ( $this->coins as $val ) {
								$apikey = $this->api_key;
								if ( ! empty( $apikey ) ) { ?>
                                    <option data-image="<?php echo WC_BlockBee_Gateway::$COIN_OPTIONS[ $val ]['logo']; ?>" value="<?php echo $val; ?>" <?php
									if ( ! empty( $selected ) && $selected === $val ) {
										echo " selected='true'";
									}
									$crypto_name = is_array( WC_BlockBee_Gateway::$COIN_OPTIONS[ $val ] ) ? WC_BlockBee_Gateway::$COIN_OPTIONS[ $val ]['name'] : WC_BlockBee_Gateway::$COIN_OPTIONS[ $val ];
									?>> <?php echo __( 'Pay with', 'blockbee' ) . ' ' . $crypto_name; ?></option>
									<?php
								}
							}
							?>
	...
	function process_payment( $order_id ) {
		...
		if ( ! empty( $apikey ) ) {
			...
			try {
				...
				error_log( $crypto_total );

				if ( $crypto_total < $min_tx ) {
					wc_add_notice( __( 'Payment error:', 'woocommerce' ) . ' ' . __( 'Value too low, minimum is', 'blockbee' ) . ' ' . $min_tx . ' ' . strtoupper( $selected ), 'error' );
				...
				$order->update_status( 'on-hold', __( 'Awaiting payment', 'blockbee' ) . ': ' . WC_BlockBee_Gateway::$COIN_OPTIONS[ $selected ] );
				$woocommerce->cart->empty_cart();
	...
	function validate_payment() {
		$data = BlockBee\Helper::process_callback( $_GET );
	...
	function thankyou_page( $order_id ) {
		...
		?>
        <div class="blockbee_payment-panel<?php
		...
		?>">
            <div class="blockbee_payment_details">
                    ...
                    <div class="blockbee_payments_wrapper">
						...
						?>; width: <?php echo intval( $this->qrcode_size ) + 20; ?>px;">
							...
							if ( $crypto_allowed_value == true ) {
                                ...
                                <div class="inner-wrapper">
                                    <figure>
										...
										if ( $qr_code_setting != 'hide_amount' ) {
											...
											?> src="data:image/png;base64,<?php echo $qr_code_img; ?>" alt="<?php echo __( 'QR Code without value', 'blockbee' ); ?>"/>
											<?php
										}
										if ( $qr_code_setting != 'hide_without_amount' ) {
											...
											?> src="data:image/png;base64,<?php echo $qr_code_img_value; ?>"
                                                 alt="<?php echo __( 'QR Code with value', 'blockbee' ); ?>"/>
											<?php
										}
										?>
                                    </figure>
									...
									if ( $qr_code_setting != 'hide_amount' && $qr_code_setting != 'hide_without_amount' ) {
										...
										if ( $qr_code_setting != 'hide_without_amount' ) {
											...
											?>" aria-label="<?php echo __( 'Show QR Code without value', 'blockbee' ); ?>">
												<?php echo __( 'ADDRESS', 'blockbee' ); ?>
                                            </button>
											<?php
										}
										if ( $qr_code_setting != 'hide_amount' ) {
											...
											?>" aria-label="<?php echo __( 'Show QR Code with value', 'blockbee' ); ?>">
												<?php echo __( 'WITH AMOUNT', 'blockbee' ); ?>
                                            </button>
                                            </div>
											<?php
										}
                                ...
                                <div class="inner-wrapper">
                                    <figure>
                                        <img class="blockbee_qrcode no_value" src="data:image/png;base64,<?php echo $qr_code_img; ?>"
                                             alt="<?php echo __( 'QR Code without value', 'blockbee' ); ?>"/>
                                    </figure>
                                    <div class="blockbee_qrcode_buttons">
                                        <button class="blockbee_qrcode_btn no_value active" aria-label="<?php echo __( 'Show QR Code without value', 'blockbee' ); ?>">
											<?php echo __( 'ADDRESS', 'blockbee' ); ?>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="blockbee_details_box">
                            <div class="blockbee_details_text">
								<?php echo __( 'PLEASE SEND', 'blockbee' ) ?>
                                <button class="blockbee_copy blockbee_details_copy" data-tocopy="<?php echo $crypto_value; ?>">
                                    <span><b class="blockbee_value"><?php echo $crypto_value ?></b></span>
                                    <span><b><?php echo strtoupper( $crypto_coin ) ?></b></span>
                                    <span class="blockbee_tooltip blockbee_copy_icon_tooltip tip"><?php echo __( 'COPY', 'blockbee' ); ?></span>
                                    <span class="blockbee_tooltip blockbee_copy_icon_tooltip success" style="display: none"><?php echo __( 'COPIED!', 'blockbee' ); ?></span>
                                </button>
                                <strong>(<?php echo "{$currency_symbol}" . " <span class='blockbee_fiat_total'>" . $total . "</span>"; ?>)</strong>
                            </div>
							...
							if ( intval( $this->refresh_value_interval ) != 0 ) {
                                ...
                                <div class="blockbee_time_refresh">
									<?php echo sprintf( __( 'The %1s conversion rate will be adjusted in', 'blockbee' ),
										strtoupper( $crypto_coin )
									); ?>
                                    <span class="blockbee_time_seconds_count" data-soon="<?php echo __( 'a moment', 'blockbee' ); ?>"
                            ...
                            <div class="blockbee_details_input">
                                ...
                                <button class="blockbee_copy blockbee_copy_icon" data-tocopy="<?php echo $address_in; ?>">
                                    <span class="blockbee_tooltip blockbee_copy_icon_tooltip tip"><?php echo __( 'COPY', 'blockbee' ); ?></span>
                                    <span class="blockbee_tooltip blockbee_copy_icon_tooltip success" style="display: none"><?php echo __( 'COPIED!', 'blockbee' ); ?></span>
                                </button>
						...
						if ( intval( $this->order_cancellation_timeout ) != 0 ) {
							?>
                            <span class="blockbee_notification_cancel" data-text="<?php echo __( 'Order will be cancelled in less than a minute.', 'blockbee' ); ?>">
                                    <?php echo sprintf( __( 'This order will be valid for %s', 'blockbee' ), '<strong><span class="blockbee_cancel_timer" data-timestamp="' . $cancel_timer . '">' . date( 'H:i', $cancel_timer ) . '</span></strong>' ); ?>
                                </span>
							<?php
						}
						?>
                        <div class="blockbee_buttons_container">
                            <a class="blockbee_show_qr" href="#" aria-label="<?php echo __( 'Show the QR code', 'blockbee' ); ?>">
								...
								?>"><?php echo __( 'Close QR CODE', 'blockbee' ); ?></span>
                            </a>
                        </div>
						<?php
						if ( $this->show_branding ) {
                            ...
                            <div class="blockbee_branding">
                                <a href="https://blockbee.io/" target="_blank">
                                    ...
                                    <img width="94" class="img-fluid" src="<?php echo BLOCKBEE_PLUGIN_URL . 'static/files/blockbee_logo.png' ?>" alt="BlockBee Logo"/>
                                </a>
                            </div>
							<?php
						}
                ...
                <div class="blockbee_payment_processing" style="display: none;">
                    ...
                    <h2><?php echo __( 'Your payment is being processed!', 'blockbee' ); ?></h2>
                    <h5><?php echo __( 'Processing can take some time depending on the blockchain.', 'blockbee' ); ?></h5>
                </div>
                ...
                <div class="blockbee_payment_confirmed" style="display: none;">
                    ...
                    <h2><?php echo __( 'Your payment has been confirmed!', 'blockbee' ); ?></h2>
                </div>
                ...
                <div class="blockbee_payment_cancelled" style="display: none;">
                    ...
                    <h2><?php echo __( 'Order has been cancelled due to lack of payment. Please don\'t send any payment to the address.', 'blockbee' ); ?></h2>
                </div>
                <div class="blockbee_history" style="display: none;">
                    <table class="blockbee_history_fill">
                        <tr class="blockbee_history_header">
                            <th><strong><?php echo __( 'Time', 'blockbee' ); ?></strong></th>
                            <th><strong><?php echo __( 'Value Paid', 'blockbee' ); ?></strong></th>
                            <th><strong><?php echo __( 'FIAT Value', 'blockbee' ); ?></strong></th>
                        </tr>
                    </table>
                </div>
				<?php
				if ( $total > 0 ) {
                    ...
                    <div class="blockbee_progress">
                        <div class="blockbee_progress_icon waiting_payment done">
                            ...
                            <p><?php echo __( 'Waiting for payment', 'blockbee' ); ?></p>
                        </div>
                        <div class="blockbee_progress_icon waiting_network">
                            ...
                            <p><?php echo __( 'Waiting for network confirmation', 'blockbee' ); ?></p>
                        </div>
                        <div class="blockbee_progress_icon payment_done">
                            ...
                            <p><?php echo __( 'Payment confirmed', 'blockbee' ); ?></p>
                        </div>
                    </div>
					<?php
				}
		...
		foreach ( $orders as $order ) {
			...
			if ( $value_refresh !== 0 && ( (int) $last_price_update + (int) $value_refresh < time() ) && ! empty( $last_price_update ) ) {
				if ( $remaining === $remaining_pending && $remaining_pending > 0 ) {
					...
					$calc_cron              = $this->calc_order( $history, $order->get_meta( 'blockbee_total' ), $order->get_meta( 'blockbee_total_fiat' ) );
	...
	function add_email_link( $order, $sent_to_admin, $plain_text, $email ) {
		...
		if ( $email->id == 'customer_on_hold_order' ) {
			WC_BlockBee_Gateway::$HAS_TRIGGERED = true;
			echo '<a style="display:block;text-align:center;margin: 40px auto; font-size: 16px; font-weight: bold;" href="' . $this->get_return_url( $order ) . '" target="_blank">' . __( 'Check your payment status', 'blockbee' ) . '</a>';
		}
	}
