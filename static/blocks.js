(() => {
    const settings = wc.wcSettings.getSetting('blockbee_data', {});
    ...
    const updateCoin = async (coin, selected = false) => {
        try {
            await apiFetch({
                path: '/blockbee/v1/update-coin',
                method: 'POST',
                headers: {
                    'X-WP-Nonce': blockbeeData.nonce,
                },
                data: {
                    coin: coin,
                    selected: selected
    ...
    const Content = ({ eventRegistration, emitResponse }) => {
        ...
        if (!settings.checkout_enabled) {
            ...
            return createElement('div', { className: 'form-row form-row-wide blockbee-payment-selection' },
    ...
    const Label = ({ components }) => {
        ...
        return React.createElement(
            'div',
            {
                className: name + '-payment-gateway blockbee-payment-selection',
            },
            iconsComp,
            labelComp
        );
